...
def search(img_path, img_db, fmt, id_to_label, train_paths):
    ...
    start_time = time.time()
    result = img_db.search(vectors_to_search, "embeddings", search_params, limit=20, output_fields=["label", "serial_no"])
    end_time = time.time()
    ...
    for hits in result:
        for hit in hits:
            ...
            l, p = hit.entity.get('label'), train_paths[hit.entity.get('serial_no')]
            res_arr.append([l,i])
            print(f"hit: {hit}, label: {id_to_label[hit.entity.get('label')]}")
